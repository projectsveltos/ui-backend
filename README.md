...
```/profile?namespace=<profile namespace>&name=<profile name>&profile_kind=<profile kind>```
...
### Get DryRun Changes for a Profile+Cluster

```/dryRunChanges?profile_kind=<kind>&profile_name=<name>&namespace=<cluster-namespace>&name=<cluster-name>&type=<cluster-type>```

Returns the simulated changes (diff) that a DryRun profile would apply to a specific cluster, based on the ClusterReport generated by the controller.

Required query parameters:

- `profile_kind` => kind of the profile: `ClusterProfile` or `Profile`
...
- `profile_namespace` => namespace of the profile (required when `profile_kind` is `Profile`)

For instance:

```
http://localhost:9000/dryRunChanges?profile_kind=ClusterProfile&profile_name=deploy-kyverno&namespace=default&name=clusterapi-workload&type=capi
```
  ...
  "helmReleaseChanges": [
    {
      ...
      "chartVersion": "v3.2.6",
      "action": "Upgrade",
      "message": "current version: v3.1.4"
    }
  ],
  "resourceChanges": [
    {
...
When the controller has not yet generated a ClusterReport for the profile+cluster pair (e.g. the profile was just created), `reportMissing` is set to `true`:
